 ...
 * 伸展树
 * 
 * 在O(log n)内完成插入、查找和删除操作。它由Daniel Sleator和Robert Tarjan创造。
 * 
...
public class SplayTree<T extends Comparable<T>> {
	...
	public class SplayTreeNode<K extends Comparable<T>> {
		T key; // 关键字(键值)
		SplayTreeNode<K> left; // 左孩子
		SplayTreeNode<K> right; // 右孩子
		...
		public SplayTreeNode(T key, SplayTreeNode<K> left, SplayTreeNode<K> right) {
			this.key = key;
			this.left = left;
			this.right = right;
		}
