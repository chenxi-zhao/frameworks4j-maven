...
//        createDocument("article", "article", "3");
//		getDocument("article", "article", "3");
...
//		clearCache();
    ...
    private static void updateDocument(String index, String type, String id) throws Exception {
        ...
        for (int i = 0; i < 5000; i++) {
            article.setTitle("中国3颗卫星拍到阅兵现场高清照" + i);
            String script = "{" +
                    "    \"doc\" : {" +
        ...
        System.out.println("更新重复索引时间：" + (System.currentTimeMillis() - begin) + " ms");
    }
     ...
     * @throws Exception
     */
    private static void createDocument(String index, String type, String id) throws Exception {
        JestClient jestClient = JestExample.getJestClient();
        Article article = new Article();
        article.setId(Integer.parseInt(id));
        article.setTitle("中国3颗卫星拍到阅兵现场高清照");
        ...
        JestResult result = null;
        for (int i = 0; i < 100; i++) {
            Index update = new Index.Builder(article).index(index).type(type).id(id).build();
            result = jestClient.execute(update);
            System.out.println(result);
        }
        System.out.println("创建重复索引时间：" + (System.currentTimeMillis() - begin) + " ms");
    }
...
//        searchSourceBuilder.highlight(highlightBuilder);
    ...
    private static void bulkIndexTest() throws Exception {
        long begin = System.currentTimeMillis();

        JestClient jestClient = JestExample.getJestClient();
        Article article = new Article(7, "张德江:发挥人大主导作用 加快完备法律规范体系", "新华网广州9月7日电 中共中央政治局常委、全国人大常委会委员长张德江9月6日至7日在广东出席第21次全国地方立法研讨会，并在佛山市就地方人大工作进行调研。他强调，要全面贯彻落实党的十八大和十八届三中、四中全会精神，深入学习贯彻习近平总书记系列重要讲话精神，认真实施立法法，充分发挥人大及其常委会在立法工作中的主导作用，抓住提高立法质量这个关键，加快形成完备的法律规范体系，为协调推进“四个全面”战略布局提供法治保障。" +
                "张德江指出，立法权是人大及其常委会的重要职权。做好新形势下立法工作，要坚持党的领导，贯彻党的路线方针政策和中央重大决策部署，切实增强思想自觉和行动自觉。", "http://news.163.com/15/0907/20/B2UGEUTJ00014PRF.html", Calendar.getInstance().getTime(), "新华网", "陈菲");

        List<Index> indexs = new ArrayList<>();
        for (int i = 0; i < 1000; i++) {
            article.setTitle("张德江:发挥人大主导作用 加快完备法律规范体系" + i);
            article.setId(i);

            indexs.add(new Index.Builder(article).id(article.getId()+"").build());
        }
